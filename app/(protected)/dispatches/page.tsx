    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3">
        {dispatches.map((d) => {
          ...
          return (
            <div key={d.id} className="flex items-center justify-between rounded border bg-white p-4">
              <div>
                <div className="font-semibold">
                  Dispatch {d.id.slice(0, 8)} — {d.status ?? 'DRAFT'}
                </div>
                <div className="text-xs text-gray-600">
                  {new Date(d.createdAt).toLocaleString()} • Items: {total} • Handed out: {handed}/{total} • Received: {received}/{total}
                </div>
                {d.items.length > 0 && (
                  <div className="mt-1 text-xs text-gray-500">
                    {d.items.slice(0, 3).map(i => i.description).join(' · ')}
                    {d.items.length > 3 ? ' …' : ''}
                  </div>
                )}
              </div>
              ...
              <div className="flex gap-2">
                <Link
                  href={`/project/${d.projectId}/dispatches/${d.id}`}
